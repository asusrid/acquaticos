  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (typeof window.ethereum !== 'undefined') {
        ...
        const balanceResult = await erc20Contract.balanceOf(await signer.getAddress());
        const supplyResult = await erc20Contract.totalSupply();
        const remainingResult = await acquaDashboardContract.remaining(await signer.getAddress());
        const pullDataByCompanyResult = await acquaDashboardContract.pullDataByCompany(await signer.getAddress());
        console.log(pullDataByCompanyResult.toArray())
